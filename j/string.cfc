component
	...
	strLog=createObject("component", "cfc.logger")
	strLog.outfile="C:/Users/Alexander Remus/Documents/log2.txt"
	...
	*/
	...
	public string function detectNewlineType(required string source){
		var list1=findAll(source,chr(13)&chr(10))
		strLog.write(serialize(list1))
		var list2=findAll(source,chr(10))
		strLog.write(serialize(list2))
		if(arrayLen(list1)>arrayLen(list2)){
	...
	public numeric function _find(required string source,required string what,numeric begin=0,numeric end=-1){
		if(end==-1){
			end=stringLen(source)-1
		}
		f= find(what,source,begin+1)-1
		if(f+stringlen(what)<=end){
			return f
		}
		return -1
	}
	
	public numeric function find(required string source,required string what,numeric begin=0,numeric end=-1){
		return _find(source,what,begin,end)
	}
	
	public array function findAll(required string source,required string what, numeric begin=0,numeric end=-1){
		f=_find(source,what,begin,end)
		r=arrayNew()
		if(f==-1){
			return r
		}
		while(f!=-1){
			arrayappend(r,f)
			f=_find(source,what,f+1,end)
		}
		return r
	}
	
	public numeric function len(required string source){
		return stringLength(source)-1
	}
	
	public array function searchRe(required string source, required string regex, numeric start=0, numeric end=-1){
		if(end==-1){
			end=len(source)
		}
		sub1=sub(source,start,end)
		ret= jreMatchGroups(sub1, regex)
		return ret
	}
	
	public array function split(required string source, required string delim,boolean keepEmpty=true,  numeric begin=0,numeric end=-1){
		if(end==-1){
			end=len(source)
		}
		s=sub(source,begin,end)
		return listToArray(source,delim,keepEmpty,true)
	}
	
	public string function sub(required string source, numeric begin = 0,numeric end=-1){
		if(end==-1){
			end=len(source)
		}
		return mid(source, begin+1,end-begin)
	}
